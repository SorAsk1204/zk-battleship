 ...
 * 不弹模态、不调 alert(§7.4 禁原生弹窗);导出体整段 try/catch,失败走可选 onError 由父级展示
 * (§7.4 不在此 alert)。在本调用点(锁定后的已校验 board 五元组 + exportBoardJSON 纯函数 + 本地构造
 * Blob)理论不会抛;onError + try/catch 是为 3.8/复用在别的上下文导出时的防御兜底,且保证 object URL
 * 在任何路径都被 revoke(finally 清理,不泄露 blob: 句柄)。
 *
...
export type ExportButtonProps = {
  ...
  commitment?: bigint;
  /** 导出失败时回调(由父级展示,§7.4 不在此 alert)。本调用点理论不抛;为复用上下文兜底。 */
...
export default function ExportButton({
  ...
  const onExport = useCallback(() => {
    ...
    try {
      ...
      // 本调用点理论不抛(已校验 board + 纯函数 + 本地 Blob);兜底交父级展示,§7.4 不在此 alert。
      onError?.(e);
    } finally {
      // 释放 object URL(无论成功/异常都回收已创建的 blob: 句柄;未创建则跳过)。
      if (url) URL.revokeObjectURL(url);
    }
  }, [chainId, contract, gameId, address, board, salt, commitment, onError]);
